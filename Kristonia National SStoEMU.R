...
clients <-  data %>% 
  ...
  mutate(type="clients") %>%
  select( type, Method_Det, Year, Method, Method_Num, users, users_unadj)
...
clients_user_group <- clients %>% mutate(Method_G=case_when(Method_Det=="Tubal Ligation (F)" ~ "Sterilization (female)",
  ...
  ungroup() %>%
  select(type,  Method_G, Year, users, users_unadj) %>% 
  group_by(  Method_G, Year) %>%
...
clients_year <- clients_user_group %>% group_by( Year) %>% 
...
EMU_Annual <- clients_year %>% rename(year=Year) %>% left_join( pop, by=c( "year")) %>% 
...
wra_mcpr_year <- full_join(pop_long, fpet, by=c("year")) %>%
  filter(year>=2005 & year<=2020) %>%
  mutate(AW_Users=Pop*mCPR_AW)

# For output graphic 2: method mix number of users
pop_long_n <- pop_long %>% mutate(n=1)
methodmix_users <- method_mix  %>% mutate(n=1) %>%
  full_join(pop_long_n, by="n") %>% select(-n) %>%
  full_join(fpet, by=c("year")) %>%
  ...
  group_by( year, Method_G) %>%
  summarise(Users_method=sum(Users_method), prev_method=sum(prev_method), MethodMix=sum(MethodMix)) %>% ungroup() %>%
  select(year, Method_G, Users_method, prev_method, MethodMix)



EMU_FPET <- left_join(EMU_Annual, wra_mcpr_year, by=c("year"))

clients_out1 <- EMU_FPET %>% select( year, EMUsers, Unadj_EMUusers, AW_Users) %>% 
...
clients_out2 <- clients_user_group %>% rename(year=Year) %>% left_join( methodmix_users, by=c( "year", "Method_G")) %>% 
  select( Method_G, year, users, users_unadj, Users_method) %>%
...
clients_out3 <- clients_user_group %>% rename(year=Year) %>% 
  select( Method_G, year, users)

clients_out5 <- clients_user_group %>% rename(year=Year) %>% left_join( methodmix_users, by=c( "year", "Method_G")) %>% 
  select(Method_G, year, users, users_unadj, Users_method) %>%
  gather(Variable, Value, users:Users_method) %>%
  mutate(Variable_num=case_when(Variable=="users" ~ 2,
                                Variable=="users_unadj" ~ 1,
  ...
  filter(Variable_num!=1) 

clients_out6 <-  clients_user_group %>% rename(year=Year) %>% full_join( EMU_FPET, by=c("year"))  %>% 
  left_join(methodmix_users, by=c("Method_G", "year")) %>% 
  mutate(Share=(users/EMUsers)*100,
         MethodMix=MethodMix*100) %>% 
  select( Method_G, year, MethodMix, Share) %>%
  mutate(Method_Num= case_when(  Method_G== "Sterilization (female)" ~ 1,
                                 Method_G== "Sterilization (male)" ~ 2,
                                 Method_G==  "IUD" ~ 3,
                                 Method_G== "Implant" ~ 4,
                                 ...
                                 Method_G==  "LAM" ~ 8,
                                 Method_G== "Other Modern Methods" ~ 9)) %>%
  arrange( year, -Method_Num) %>%
  gather(Variable, Value, MethodMix:Share) %>% 
  group_by( year, Variable) %>% mutate(pos = cumsum(Value)- Value/2) %>%
...
clients_out7 <- left_join(EMU_FPET, dhs_mcpr, by=c( "year")) %>%
  mutate(emu_clients_incCon=emu_clients_incCon*100,
         emu_clients_exCon=emu_clients_exCon*100,
         mCPR=mCPR_AW.x * 100,
         DHS_aw_mCPR = mCPR_AW.y * 100) %>%
  select( year, emu_clients_incCon, emu_clients_exCon, mCPR, DHS_aw_mCPR) %>%
...
clients_out8 <- clients_out7 %>% group_by( Variable) %>% summarise(Ave_Change=mean(annual_change, na.rm=TRUE)) %>%
...
emu_trends <-EMU_Annual  %>%  select(year, emu_clients_incCon, emu_clients_exCon)  %>%
  left_join(fpet, by=c( "year"))  %>% 
  rename(UNPD_mCPR=mCPR_AW) %>% select(-mCPR_MW) %>%
  left_join(dhs_mcpr, by=c( "year"))   %>% select(-mCPR_MW) %>% rename(DHS_aw_mCPR=mCPR_AW) %>%
  gather(Variable, Value, emu_clients_incCon:DHS_aw_mCPR) %>%
...
emu_annual_change <- emu_trends %>% group_by( Variable) %>% summarise(Ave_Change=mean(annual_change, na.rm=TRUE)) %>%
  mutate(Ave_Change_Label=case_when(is.na(Ave_Change) ~ "No Data",
                                    !is.na(Ave_Change) ~ paste(round(Ave_Change, 1), "%", sep="")),
         Ave_Change=case_when(is.na(Ave_Change) ~ 0,
                              !is.na(Ave_Change) ~ Ave_Change)) %>%
...
clients_emu_users <- clients_user_group %>% rename(year=Year) %>% select( Method_G, year, users) %>% rename(clients_users=users)
survey_users <- methodmix_users %>% select(  Method_G, year, Users_method) %>% rename(survey_users=Users_method)

emu_users <- clients_emu_users %>% 
  left_join(survey_users, by=c( "Method_G", "year")) %>%
...
# The following code creates PDF a document for all graphics

region_name <- "Kristonia"

  #################################################################
  # Input 
  ################################################################
  
  g_clients_input <- ggplot(values_input, aes(x=Year, y=Value, fill=Method_Det_name))+
  ...
  g_clients_out1 <-   ggplot(clients_out1, aes(x=year, y=Value, color=Variable_name))+
    geom_point(size=4)+
    geom_line(size=2)+
  ...
  g_clients_out2 <-  ggplot(subset(clients_out2, year==2020), aes(x=fct_reorder(Variable_name, -Variable_num), y=Value, fill=fct_reorder(Method_G, -Method_Num)))+
    geom_bar(stat="identity") +
    coord_flip() +
...
#################################################################
  ...
  g_clients_out3 <-    ggplot(clients_out3, aes(x=year, y=users, color=Method_G))+
    geom_line(size=2)+
  ...
  g_clients_out4 <-   ggplot(subset(clients_out2, Year==2020 & Variable_num!=1), aes(x=fct_reorder(Variable_name, -Variable_num), y=Value, fill=fct_reorder(Method_G, -Method_Num)))+
    geom_bar(stat="identity") +
    coord_flip() +
    scale_fill_manual(breaks= c("Sterilization (female)" ,
                                "Sterilization (male)" ,  
  ...
  g_clients_out5 <-   ggplot(subset(clients_out5, year==2020 ), aes(x=fct_reorder(Method_Name, Method_Num), y=Value, fill=fct_reorder(Variable_name, Variable_num)))+
  ...
  g_clients_out6 <-   ggplot(subset(clients_out6, year==2020), aes(x="", y=Value, fill=fct_reorder(Method_G, Method_Num)))+
  ...
  g_clients_out7 <-    ggplot(clients_out7, aes(x=year, y=Value, color=Variable_name))+
    geom_point(size=5)+
    geom_line(size=2) +
  ...
  g_clients_out8 <-   ggplot(clients_out8, aes(x=fct_reorder(Variable_name, Variable_num), y=Ave_Change, fill=Variable_name))+
    geom_bar(stat="identity") +
  ...
  # EMU Outputs 
  ################################################################
  g_emu_1 <-     ggplot(emu_trends, aes(x=year, y=Value, color=Variable_name))+
    geom_point(size=4)+
    geom_line(size=2) +
  ...
  g_emu_2 <- ggplot(emu_annual_change, aes(x=fct_reorder(Variable_name, Variable_num), y=Ave_Change, fill=Variable_name))+
    geom_bar(stat="identity") +
    geom_text(aes(label=Ave_Change_Label, hjust=hjust_val), position=position_dodge(width=0.9), vjust=0.5) +
    coord_flip() +
  ...
  g_emu_3 <-  ggplot(subset(emu_users, year==2020), aes(x=Variable_Name, y=Value, fill=fct_reorder(Method_G, -Method_Num)))+
    geom_bar(stat="identity") +
    coord_flip() +
    scale_fill_manual(breaks= c("Sterilization (female)" ,
                                "Sterilization (male)" ,  
  ...
  g_emu_4 <-  ggplot(subset(emu_users, Variable!="survey_users"), aes(x=year, y=Value, color=Variable_Name)) +
    geom_line(size=1) +
  ...
  ggexport(multi.page, filename=paste("Results/Kristonia National_SStoEMU Results.pdf", sep=""), height=8, width=12)
  


# Export EMU csv files
write.csv(emu_trends_wide, "Results/EMU National Kristonia.csv", row.names = F, na="")
write.csv(EMU_Annual, "Results/EMU Totals National Kristonia.csv", row.names = F, na="")
