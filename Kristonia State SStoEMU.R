# Kristonia State (Level 1) SStoEMU
...
inputs <- read.csv("Kristonia Annual State Service Statistics Input Data.csv") 
pop <- read.csv("Kristonia State Population Data.csv") %>% rename(year=Year)
FPSource <- read.csv("Kristonia State FPSource.csv") 

fpet <- read.csv("FPET Results for State Kristonia.csv")
method_mix <- read.csv("Kristonia State Method Mix Data.csv")
dhs_mcpr <- read.csv("Fake DHS Results for State Kristonia.csv")
pop_long <- read.csv("Population 2000 2030 for State Kristonia.csv")
...
country_sector_rep <- full_join(FPSource_clean, Sectors_reporting_clean, by="Source") %>%
  ...
  spread(State, Adjustment_Factor) %>%
  full_join(method.df, by="Method") %>% 
  gather(State, Adjustment_Factor, State1:State2)  %>% 
...
lam_first <- data %>%   full_join( cyp, by="Method_Det") %>% 
  ...
  select(State, Method_Det,  Value) %>% rename(LAPM_First=Value)
...
clients <-  data %>% 
  ...
  full_join( country_sector_rep, by=c( "Method_Num", "State")) %>%
  full_join(continuation_lapm, by=c("Method_Det", "Year", "State")) %>%
  ...
  arrange(State, Method_Det, Year) %>%
  group_by(State, Method_Det) %>% 
  ...
  mutate(type="clients") %>%
  select(State, type, Method_Det, Year, Method, Method_Num, users, users_unadj)
...
clients_user_group <- clients %>% mutate(Method_G=case_when(Method_Det=="Tubal Ligation (F)" ~ "Sterilization (female)",
  ...
  ungroup() %>%
  select(State, type,  Method_G, Year, users, users_unadj) %>% 
  group_by( State, Method_G, Year) %>%
...
clients_year <- clients_user_group %>% group_by(State, Year) %>% 
...
EMU_Annual <- clients_year %>% rename(year=Year) %>% left_join( pop, by=c("State", "year")) %>% 
...
wra_mcpr_year <- full_join(pop_long, fpet, by=c("year", "State")) %>%
...
methodmix_users <- method_mix  %>%
  full_join(pop_long, by="State") %>%
  full_join(fpet, by=c("year", "State")) %>%
  ...
  group_by(State, year, Method_G) %>%
  summarise(Users_method=sum(Users_method), prev_method=sum(prev_method), MethodMix=sum(MethodMix)) %>% ungroup() %>%
  select(State, year, Method_G, Users_method, prev_method, MethodMix)



EMU_FPET <- left_join(EMU_Annual, wra_mcpr_year, by=c("year", "State"))

clients_out1 <- EMU_FPET %>% select(State, year, EMUsers, Unadj_EMUusers, AW_Users) %>% 
...
clients_out2 <- clients_user_group %>% rename(year=Year) %>% left_join( methodmix_users, by=c("State", "year", "Method_G")) %>% 
  select(State, Method_G, year, users, users_unadj, Users_method) %>%
  ...
  mutate(Method_Num= case_when(  Method_G== "Sterilization (female)" ~ 1,
                                 ...
                                 Method_G==  "LAM" ~ 8,
                                 Method_G== "Other Modern Methods" ~ 9)) %>% filter(!is.na(State))


clients_out3 <- clients_user_group %>% rename(year=Year) %>% 
  select(State, Method_G, year, users)

clients_out5 <- clients_user_group %>% rename(year=Year) %>% left_join( methodmix_users, by=c("State", "year", "Method_G")) %>% 
  select(State, Method_G, year, users, users_unadj, Users_method) %>%
  gather(Variable, Value, users:Users_method) %>%
  mutate(Variable_num=case_when(Variable=="users" ~ 2,
                                Variable=="users_unadj" ~ 1,
  ...
  filter(Variable_num!=1) %>% filter(!is.na(State))

clients_out6 <-  clients_user_group %>% rename(year=Year) %>% full_join( EMU_FPET, by=c("State", "year"))  %>% 
  left_join(methodmix_users, by=c("State", "Method_G", "year")) %>% 
  mutate(Share=(users/EMUsers)*100,
         MethodMix=MethodMix*100) %>% 
  select(State, Method_G, year, MethodMix, Share) %>%
  mutate(Method_Num= case_when(  Method_G== "Sterilization (female)" ~ 1,
                                 Method_G== "Sterilization (male)" ~ 2,
                                 Method_G==  "IUD" ~ 3,
                                 Method_G== "Implant" ~ 4,
                                 ...
                                 Method_G==  "LAM" ~ 8,
                                 Method_G== "Other Modern Methods" ~ 9)) %>%
  arrange(State, year, -Method_Num) %>%
  gather(Variable, Value, MethodMix:Share) %>% 
  group_by(State, year, Variable) %>% mutate(pos = cumsum(Value)- Value/2) %>%
...
clients_out7 <- left_join(EMU_FPET, dhs_mcpr, by=c("State", "year")) %>%
  mutate(emu_clients_incCon=emu_clients_incCon*100,
         emu_clients_exCon=emu_clients_exCon*100,
         mCPR=mCPR_AW.x * 100,
         DHS_aw_mCPR = mCPR_AW.y * 100) %>%
  select(State, year, emu_clients_incCon, emu_clients_exCon, mCPR, DHS_aw_mCPR) %>%
...
clients_out8 <- clients_out7 %>% group_by(State, Variable) %>% summarise(Ave_Change=mean(annual_change, na.rm=TRUE)) %>%
...
emu_trends <-EMU_Annual  %>%  select(State, year, emu_clients_incCon, emu_clients_exCon)  %>%
  left_join(fpet, by=c("State", "year"))  %>% 
  rename(UNPD_mCPR=mCPR_AW) %>% select(-mCPR_MW) %>%
  left_join(dhs_mcpr, by=c("State", "year"))   %>% select(-mCPR_MW) %>% rename(DHS_aw_mCPR=mCPR_AW) %>%
  gather(Variable, Value, emu_clients_incCon:DHS_aw_mCPR) %>%
...
emu_annual_change <- emu_trends %>% group_by(State, Variable) %>% summarise(Ave_Change=mean(annual_change, na.rm=TRUE)) %>%
  mutate(Ave_Change_Label=case_when(is.na(Ave_Change) ~ "No Data",
                                    !is.na(Ave_Change) ~ paste(round(Ave_Change, 1), "%", sep="")),
         Ave_Change=case_when(is.na(Ave_Change) ~ 0,
                              !is.na(Ave_Change) ~ Ave_Change)) %>%
...
clients_emu_users <- clients_user_group %>% rename(year=Year) %>% select(State,  Method_G, year, users) %>% rename(clients_users=users)
survey_users <- methodmix_users %>% select(State,  Method_G, year, Users_method) %>% rename(survey_users=Users_method)

emu_users <- clients_emu_users %>% 
  left_join(survey_users, by=c("State", "Method_G", "year")) %>%
...
levels(as.factor(emu_users_wide$State))
# Graphics
regions <- c( "State1", "State2"  )
...
for (row in 1:nrow(regions.df)) {
  ...
  clients_input_reg <- values_input %>% filter(State==region_name)
  
  clients_out1_reg <- clients_out1 %>% filter(State==region_name)
  clients_out2_reg <- clients_out2 %>% filter(State==region_name)
  clients_out3_reg <- clients_out3 %>% filter(State==region_name)
  clients_out5_reg <- clients_out5 %>% filter(State==region_name)
  clients_out6_reg <- clients_out6 %>% filter(State==region_name)
  clients_out7_reg <- clients_out7 %>% filter(State==region_name)
  clients_out8_reg <- clients_out8 %>% filter(State==region_name)
  
  emu_trends_reg <- emu_trends  %>% filter(State==region_name)
  emu_annual_change_reg <- emu_annual_change %>% filter(State==region_name)
  emu_users_reg <- emu_users %>% filter(State==region_name)
  ...
  ggexport(multi.page, filename=paste("Results/KristoniaStateLevel1_" , region_name,  "_SStoEMU Results.pdf", sep=""), height=8, width=12)
  
}


# Can use the code below to combine all the regions' PDFs together
pdf_combine(c("Results/KristoniaStateLevel1_State1_SStoEMU Results.pdf" ,
              "Results/KristoniaStateLevel1_State2_SStoEMU Results.pdf"   ), output = "Results/Kristonia State Level1 EMU Results.pdf")

# Export EMU csv files
write.csv(emu_trends_wide, "Results/EMU State Kristonia.csv", row.names = F, na="")
write.csv(EMU_Annual, "Results/EMU Totals State Kristonia.csv", row.names = F, na="")
